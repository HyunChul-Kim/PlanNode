...
class ExampleUnitTest {
     ...
     */
    private fun mostReceivedGift() {
        val question = Questions.mostReceivedGiftQuestion()
        question.case.forEach { case ->
            val answer = Solutions.mostReceivedGift(case.args.first, case.args.second)
            assertEquals(answer, case.answer)
        }
    }

    private fun donutAndBarGraph() {
        val question = Questions.donutAndBarGraphQuestion()
        question.case.forEach { case ->
            val answer = Solutions.donutAndBarGraph(case.args)
            assertArrayEquals(answer, case.answer)
        }
    }

    private fun choiceDice() {
        val question = Questions.choiceDice()
        question.case.forEach { case ->
            val answer = Solutions.pickDice(case.args)
            //assertArrayEquals(answer, case.answer)
        }
    }
}